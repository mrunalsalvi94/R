...
df1 <- read.csv('C:/Users/Mrunal/Downloads/R A-Z course/Section6-Homework-Data.csv')
head(df1)

# colnames(df1) <- c("Week","Director","Genre","Movie","Date","Studio",'Grossmillion',"Budgetmillion","IMDBrating","LensRating",
#                    "Overseasgrossmillion","Profitmillion","Profit%","Runtimemins","USgrossmillion",
#                    "grossUS%")
colnames(df1)
...
#1 
#Filtering data: 
#filter(x -> x["Genre"] in.["action", "adventure","animation","comedy","drama"], df1)

df1.1 <- df1[df1$Genre %in% c("action","adventure","animation","comedy","drama") & 
               df1$Studio %in% c("Buena Vista Studios","Fox","Sony","WB","Universal","Paramount Pictures"),]
summary(df1.1)

g <- ggplot(data=df1.1,aes(x=Genre,y=Gross...US))
h <- g + 
