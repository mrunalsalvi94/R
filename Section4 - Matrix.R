...
#Creating a data and making sure rows*columns is equal to the number of elements
my.data <- 1:25
matrixA <- matrix(my.data,5,5,byrow=T)
matrixA

my.data <- 1:25
matrixB <- matrix(my.data,5,5,byrow=F)
matrixB
...
matrixC <- rbind(r1,r2,r3)
matrixC

matrixD <- cbind(r1,r2,r3)
matrixD
...
# Naming matrix dimensions - row and column names
...
vec1 <- rep(c('a','b','c'), times=3)
vec1

vecmat1 <- matrix(vec1, 3, 3)
vecmat1

rownames(vecmat1) <- c('x','y','z')
colnames(vecmat1) <- c(1,2,3)
...
#1. Matrix operations - Goals per game - add, subtract, multiply, divide
...
#Visualization - Matplot - use t for transpose
...
#c is used to get matrix or else it is a vector
is.matrix(Games[1,5])

# to get matrix if we dont want to use c
Games[1,,drop=F]

#visualising subsets - first make a subset and make sure legend is adjusted to the number of records

d1 <- MinutesPlayed[1:3,]
matplot(t(d1),type='b',pch=15:18, col=c(1:4,6))
legend('bottomleft',inset=0.01, legend=Players[1:3],pch=15:18, col=c(1:4,6))

d2 <- MinutesPlayed[5,,drop=F]
matplot(t(d2),type='b',pch=15:18, col=c(1:4,6))
legend('bottomleft',inset=0.01, legend=Players[5],pch=15:18, col=c(1:4,6))


#Making functions

kobe <- function(){
  d3 <- MinutesPlayed[1,,drop=F]
  matplot(t(d3),type='b',pch=15:18, col=c(1:4,6))
  legend('bottomleft',inset=0.01, legend=Players[1],pch=15:18, col=c(1:4,6))
}
kobe()

#making use of rows
userows <- function(rows){
  d4 <- MinutesPlayed[rows,,drop=F]
  matplot(t(d4),type='b',pch=15:18, col=c(1:4,6))
  legend('bottomleft',inset=0.01, legend=Players[rows],pch=15:18, col=c(1:4,6))
}
userows(1:4)

#making use of rows and multiple datasets you want
userowsdata <- function(data, rows){
  d5 <- data[rows,,drop=F]
  matplot(t(d5),type='b',pch=15:18, col=c(1:4,6))
  legend('bottomleft',inset=0.01, legend=Players[rows],pch=15:18, col=c(1:4,6))
}
userowsdata(FieldGoals, 1:3)
userowsdata(Salary)
userowsdata(MinutesPlayed/Games)


#analysis of data
userowsdata(Salary)
userowsdata(Salary/Games)
userowsdata(Salary/FieldGoals)

userowsdata(MinutesPlayed)
userowsdata(Points)

userowsdata(FieldGoals/FieldGoalAttempts)
userowsdata(FieldGoalAttempts/Games)
userowsdata(Points/Games)
...
freethrow <- rbind(KobeBryant_FT,JoeJohnson_FT,LeBronJames_FT,CarmeloAnthony_FT,DwightHoward_FT,
...
datavis <- function(data, rows){
  d6 <- data[rows,,drop=F]
  matplot(t(d6),type='b',pch=15:18, col=c(1:4,6))
  legend('bottomleft',inset=0.01, legend=Players[rows],pch=15:18, col=c(1:4,6))
}
datavis(freethrow)
...
datavis(freethrowattempts/Games)


#2 accuracy x

datavis(freethrow/freethrowattempts)

#3 Playing style excluding 1 point free throws( Points - freethrow )/ field goals

datavis((Points-freethrow)/FieldGoals)
